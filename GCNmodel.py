...
from torch_geometric.nn import GCNConv, LayerNorm, GATConv, TransformConv
...
#     def __init__(self, in_dim=15962, num_hiddens=[512, 64], heads=1, concat=True,
#                 dropout=0.2, beta=0):
#         super().__init__()
#         self.conv1 = GCNConv(in_dim, num_hiddens[0], heads=heads, concat=concat,
#             dropout=dropout, beta=beta)
#         self.conv_lists = nn.ModuleList(
#             [GCNConv(num_hiddens[i-1], num_hiddens[i], heads=heads, concat=concat, dropout=dropout, beta=beta) for i in range(1, len(num_hiddens))])
#         # self.norm_lists = nn.ModuleList(
#         #     [LayerNorm(i) for i in num_hiddens[1:]])
#         self.activate = nn.ReLU(True)
#         # self.norm1 = LayerNorm(num_hiddens[0])
...
class Feature_extractor(nn.Module):
	def __init__(self, in_dim=15962, ConvF):
		super().__init__()
		self.conv1 = ConvF(in_dim, 512)
		self.conv2 = ConvF(512, 64)
		self.activate = F.elu
...
class MLP(nn.Module):
	def __init__(self, in_dim, out_dim, last=False, ConvF=TransformConv):
		super().__init__()
...
class Base_classfier(nn.Module):
	def __init__(self, num_hiddens=[64, 32, 512, 512], out_dim=2):
		super().__init__()
		self.layer_list = nn.ModuleList()
		for i in range(1, len(num_hiddens)):
			self.layer_list.append(MLP(num_hiddens[i-1], num_hiddens[i]))
		self.layer_list.append(MLP(num_hiddens[-1], out_dim))
...
class SRC_classifier(nn.Module):
	def __init__(self, num_classes=2, in_dim=15962, num_hiddens=[512, 64]):
		super().__init__()
		# self.shareNet = Feature_extractor(in_dim, num_hiddens)
		self.shareNet = Feature_extractor(in_dim)

		# self.bottleneck = MLP(num_hiddens[-1], 32)
		# here make some change to the original DAAN
		self.classifier =Base_classfier(num_hiddens=[64, 32, 16], out_dim=2)
...
class TL_classifier(nn.Module):
	def __init__(self, num_classes=2, in_dim=15962, num_hiddens=[512, 64]):
		super().__init__()
		self.shareNet = Feature_extractor(in_dim)
		self.src_classifier = Base_classfier(num_hiddens=[64, 32, 16], out_dim=num_classes)

		self.global_domain_classifier = Base_classfier(num_hiddens=[64, 32, 16], out_dim=num_classes)

		self.dcis = nn.ModuleList()
		for nc in range(num_classes):
			# out_dim = 2, one for src, one for target
			adv_classifier = Base_classfier(num_hiddens=[64, 32, 16], out_dim=2)
	...
	def forward(self, src_x, src_edge, tar_x, tar_edge, alpha=0.0):
		...
		return src_pred, src_domain_out, tar_domain_out, s_out, t_out


class TLGCNModel(nn.Module):
	...
	def inference(self):
		self.model.eval()
		self.pred, _, _, _, _ = self.model(self.tar.x, self.tar.edge_index, self.tar.x, self.tar.edge_index)
		return self.pred.argmax(dim=1)

	def forward(self):
		self.model.train()
		self.pred, self.src_domain_out, self.tar_domain_out, self.s_out, self.t_out = self.model(self.src.x, self.src.edge_index, self.tar.x, self.tar.edge_index)
	...
	def compute_loss(self):
		...
		self.dc = self.dc + tmpd_c.cpu().item()
		self.dm = self.dm + 2 * (1 - 2 * global_loss.cpu().item())
...
# 		self.model = DAANNet(num_classes=2, in_dim=15962, num_hiddens=[512, 64])
# 		self.lr = 1e-3
# 		# diff lr
# 		self.optimizer = torch.optim.SGD(model.parameters(), lr=self.lr, momentum=0.9, weight_decay=0.99)
# 		self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

# 	def set_input(self, src_data, tar_data):
# 		self.src_data = src_data.to(self.device)
# 		self.tar_data = tar_data.to(self.device)

# 	def forward(self):
# 		source, s_domain_output, t_domain_output, s_out, t_out = self.model(self.src_data, self.tar_data)
